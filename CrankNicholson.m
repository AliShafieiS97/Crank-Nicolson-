...
for i=2:N-1
    C(i,i-1) = -alpha*(deltat/(2*deltax^2)); 
...
for t = 2:M
    B(:) = T;
    for i = 2:N-1
        B(i) = alpha*(deltat/(2*deltax^2)) * T(i-1) + (1-alpha*(deltat/deltax^2)) * T(i) + alpha*(deltat/(2*deltax^2)) * T(i+1);
    end
    B(1) = 100;  
    B(N) = 20;  
...
disp ("T(x=0.75,t=600): ");
disp(T1(600/deltat,round(0.75/deltax)));